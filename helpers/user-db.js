...
import { USER_ROLE, EMPLOYEE_ROLE } from './role-constants.js';
...
export const createEmployeeUser = async (userData) => {
  ...
  try {
    ...
    // Crear el usuario principal como activo desde el inicio
    const user = await User.create(
      {
        Name: name,
        Email: email.toLowerCase(),
    ...
    // Asignar rol EMPLOYEE_ROLE obligatorio
    ...
    if (employeeRole) {
      ...
      throw new Error(
        'EMPLOYEE_ROLE not found in database during employee creation'
      );
    }
    ...
    await transaction.rollback();
    console.error('Error creando usuario empleado:', error);
    throw new Error('Error al crear usuario empleado');
  }
};
